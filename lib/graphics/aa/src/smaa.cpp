...
#include "asset/shader/smaa-pass1.frag.hpp"
#include "asset/shader/smaa-pass2.frag.hpp"
#include "asset/shader/smaa-pass3.frag.hpp"
...
namespace graphics::aa
{
	...
	std::expected<SMAA, util::Error> SMAA::create(SDL_GPUDevice* device, SDL_GPUTextureFormat format) noexcept
	{
		auto blend_lut = generate_ortho_area_lut(device, lut_size);
		if (!blend_lut) return blend_lut.error().forward("Create Area LUT failed");

		auto diag_lut = generate_diagonal_area_lut(device, lut_size);
		if (!diag_lut) return diag_lut.error().forward("Create Diagonal LUT failed");
		...
		auto shader1 = gpu::GraphicsShader::create(
			device,
			std::as_bytes(shader_asset::smaa_pass1_frag),
		...
		auto pass1 = std::move(shader1).and_then([device](gpu::GraphicsShader shader) {
			return FullscreenPass<true>::create(
				...
				edge_texture_format,
				{},
				"SMAA Pass 1 Pipeline"
			);
		});

		auto shader2 = gpu::GraphicsShader::create(
			device,
			std::as_bytes(shader_asset::smaa_pass2_frag),
			gpu::GraphicsShader::Stage::Fragment,
			2,
			0,
			0,
		...
		auto pass2 = std::move(shader2).and_then([device](gpu::GraphicsShader shader) {
			return FullscreenPass<true>::create(
				...
				{},
				"SMAA Pass 2 Pipeline"
			);
		});

		auto shader3 = gpu::GraphicsShader::create(
			device,
			std::as_bytes(shader_asset::smaa_pass3_frag),
			gpu::GraphicsShader::Stage::Fragment,
			2,
			0,
			0,
		...
		auto pass3 = std::move(shader3).and_then([&](gpu::GraphicsShader shader) {
			return FullscreenPass<true>::create(
				...
				"SMAA Pass 3 Pipeline"
			);
		});

		if (!pass1) return pass1.error().forward("Create SMAA Pass 1 failed");
		if (!pass2) return pass2.error().forward("Create SMAA Pass 2 failed");
		if (!pass3) return pass3.error().forward("Create SMAA Pass 3 failed");
	...
	SMAA::SMAA(
		...
		diag_lut(std::move(diag_lut)),
		pass1(std::move(pass1)),
		pass2(std::move(pass2)),
		pass3(std::move(pass3)),
		edge_texture(edge_texture_format, "SMAA Edge Texture"),
		blend_texture(blend_texture_format, "SMAA Blend Texture")
	{}

	std::expected<void, util::Error> SMAA::run_antialiasing(
