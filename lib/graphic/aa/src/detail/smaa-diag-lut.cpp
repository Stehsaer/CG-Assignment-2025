#include "graphic/aa/detail/smaa-diag-lut.hpp"
...
namespace graphic::aa
{
	namespace
	{
		...
		constexpr auto diagonal_subtexture_index_list = std::to_array<glm::u8vec2>({
			{0, 0},
			{1, 0},
			{0, 2},
			{1, 2},
	...
	) noexcept
	{
		...
		return graphic::create_texture_from_image(device, lut_texture_format, image);
	}
}